function [ OptionValue, Delta, Gamma ] = Function_BarrierOption_OutInOption(  model, OptionParameters, N_FouriersTerms, Control_Limit   )
...
lB = log( LowerBoundary   );
uB = log( UpBarrier );
strike_X_Space = log( K );
...
if InOutIndicator  == -1 %% -1 means out option; 1 means in-option
    %% for an out-option, the valid perid = [lB, uB]
    %% The code below is for the case double-barrier-out options
...
elseif InOutIndicator  == 1  %Here we assue it is an up-in option
    % up and in option, the valid perid = [ uB, +infty]
    
    
    %% If it is a put option
    if CallPutIndicator < 0
        l = max( [ a,   uB  ]);
        h = min( [b, strike_X_Space  ] );
        %% if d < c; then we set d=c;
        h = max( l, h );
        [ payoff_basis_integration ] =   f_k_expansion(   a, b, l, h, K,  S0,     N_FouriersTerms    ) ;
        payoff_basis_integration = - payoff_basis_integration;
...
[  A_j, Delta_A_j] = A_j_Vector(   a, b, x0,  DeltaT,   N_FouriersTerms,   model, OptionParameters    );
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
