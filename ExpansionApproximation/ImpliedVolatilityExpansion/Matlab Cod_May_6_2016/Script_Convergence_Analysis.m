...
% VanillaCallOption =     CosineMethodCallPricingFFT( 5000, 20 , model, S, K, T, r, d);
...
DownOutOption = zeros(size(N));

benchmark = Function_BarrierOption_August2014(  model, OptionParameters, 8000, Control_Limit   );

resultBarrierOutOptions = zeros( size(N) );
relativeError = zeros( size(N) );

for index = 1: length(N)
    N_FouriersTerms = N( index );
    tic
    [ resultBarrierOutOptions(index) ] =  Function_BarrierOption_August2014(  model, OptionParameters, N_FouriersTerms, Control_Limit   );
    toc
    
    relativeError(index)   =    resultBarrierOutOptions(index) / benchmark - 1;
end


plot( N, resultBarrierOutOptions )
% plot( log(N)/log(2), resultBarrierOutOptions )
hold on

plot( N, relativeError )
...
% xlabel('M (The total number of terms in the Fourier-cosine series)')
% ylabel('Prices of the down?and?out call options')
% ylabel('Prices of the down?and?out call options')
% ylabel('Prices of the down-and-out call options')
% title('Prices of down-and-out call options with different monitoring frequencies and down barriers')
%  legend('B_L=97, N=5', 'B_L=97, N=25', 'B_L=99, N=5', 'B_L=99, N=25')
%  legend('N=2', 'N= 10', 'N= 52', 'N= 252')
