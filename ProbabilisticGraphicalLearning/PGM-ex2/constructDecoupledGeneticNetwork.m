...
for i=1:numPeople
  if ~all(pedigree.parents(i,:))
    ...
    factorList(numPeople+i).var = numPeople+i;
    factorList(numPeople+i).card = numAlleles;
    factorList(numPeople+i).val = alleleFreqs;
  else
    parentOne = pedigree.parents(i,1);
    parentTwo = pedigree.parents(i,2);
    
    factorList(i).var = [i,parentOne,numPeople+parentOne];
    factorList(i).card = [numAlleles,numAlleles,numAlleles];
    for j=1:prod(factorList(i).card)
      assignment = IndexToAssignment(j,factorList(i).card);
      prob = length(find(assignment(2:end)==assignment(1)))./length(assignment(2:end));
      factorList(i).val(j) = prob;
    end
    
    
    factorList(numPeople+i).var = [numPeople+i,parentTwo,parentTwo+numPeople];
    factorList(numPeople+i).card = [numAlleles,numAlleles,numAlleles];
    for j=1:prod(factorList(numPeople+i).card)
      assignment = IndexToAssignment(j,factorList(numPeople+i).card);
      prob = length(find(assignment(2:end)==assignment(1)))./length(assignment(2:end));
